...
1. [Install Go](https://go.dev/doc/install) 1.26.2
1. Clone this repo
...
> [!CAUTION]
> If you are using the multiplexer binary (default `make build`), the embedded v7 binary requires glibc >= 2.38. This means **Ubuntu 24.04 or later** (or an equivalent distribution) is required. Nodes running older OS versions (e.g., Ubuntu 22.04) will fail to start with a glibc version mismatch error.
