...
ENC="$(hostname | sed 's/^encoder-//')"
mkdir -p "/root/logs/$TAG"
for p in $(seq 0 $((PROCS - 1))); do
  tmux new-session -d -s "txsim-$p" \
    "GOMEMLIMIT=${GOMEMLIMIT:-off} fibre-txsim --chain-id talis-fibre-r2 --grpc-endpoint $ENDPOINT --keyring-dir encoder-payload/encoder-$ENC \
      --key-prefix enc$ENC --key-offset $((p * CONCURRENCY)) --concurrency $CONCURRENCY \
