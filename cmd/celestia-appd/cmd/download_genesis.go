...
func downloadGenesisCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return downloadGenesis(chainID, outputFile)
		},
	}
...
// downloadGenesis downloads the genesis file for the specified chain ID to the given output file path.
// It validates the chain ID, downloads the file from the networks repo, and verifies the SHA-256 hash.
func downloadGenesis(chainID, outputFile string) error {
...
// chainIDs returns a comma-separated string of all known chain IDs.
func chainIDs() string {
	return strings.Join(getKeys(chainIDToSha256), ", ")
}

// getChainIDOrDefault returns the chainID from the provided arguments or defaults to "celestia".
