...
// uploadParamState models changes to the chain's admission parameters.
type uploadParamState struct {
	state.Client
	params *atomic.Pointer[fibretypes.Params]
}
...
func TestClientServerUploadLimitsChange(t *testing.T) {
	var params atomic.Pointer[fibretypes.Params]
	initial := fibretypes.DefaultParams()
	initial.MinUploadSize, initial.MaxUploadSize = 32<<20, 64<<20
	params.Store(&initial)
	env := makeTestEnv(t, 2, 1, nil, func(cfg *fibre.ServerConfig) {
		inner := cfg.StateClientFn
		cfg.StateClientFn = func() (state.Client, error) {
			client, err := inner()
			return uploadParamState{client, &params}, err
		}
	})
	defer env.Close()
	cfg := env.clients[0].Config
	inner := cfg.StateClientFn
	cfg.StateClientFn = func() (state.Client, error) {
		client, err := inner()
		return uploadParamState{client, &params}, err
	...
	// A client using legacy defaults cannot upload below the current minimum.
	small, err := fibre.NewBlob([]byte("small"), fibre.DefaultBlobConfigV0())
	require.NoError(t, err)
	defer small.Free()
	_, err = client.Upload(t.Context(), testNamespace, small)
	require.ErrorContains(t, err, "upload size")

	blobCfg, err := client.UploadBlobConfig(t.Context())
	require.NoError(t, err)
	blob, err := fibre.NewBlob([]byte("padded"), blobCfg)
	require.NoError(t, err)
	defer blob.Free()
	_, err = client.Upload(t.Context(), testNamespace, blob, fibre.WithAwaitAllSignatures())
	require.NoError(t, err)
	client.Await()

	// Lowering the maximum rejects new uploads but preserves old downloads.
	next := fibretypes.DefaultParams()
	next.MaxUploadSize = 16 << 20
	params.Store(&next)
	...
	require.Equal(t, blob.Data(), downloaded.Data())
	require.Equal(t, 32<<20, downloaded.UploadSize())
	require.Equal(t, blob.RowSize(), downloaded.RowSize())
}
