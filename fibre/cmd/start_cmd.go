...
func newStartCmd(start func(context.Context, fibre.ServerConfig) error) *cobra.Command {
	...
	cmd.Flags().BoolVar(&cfg.ObjectStorage.OverrideNamespace, flagOverrideObjectNamespace, false, "accept a migrated object storage namespace or adopt one for an older store; does not migrate or verify objects")
