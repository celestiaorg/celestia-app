...
func Test_processCompactShares(t *testing.T) {
	...
	const exactTxShareSize = appconsts.CompactContinuationShareContentSize - 1
	...
	tests := []test{
		{"single small tx", appconsts.CompactContinuationShareContentSize / 8, 1},
		{"many small txs", appconsts.CompactContinuationShareContentSize / 8, 10},
		{"single big tx", appconsts.CompactContinuationShareContentSize * 4, 1},
		{"many big txs", appconsts.CompactContinuationShareContentSize * 4, 10},
...
func TestCompactShareContainsInfoByte(t *testing.T) {
	...
	txs := generateRandomTransaction(1, appconsts.CompactContinuationShareContentSize/4)
...
func Test_parseCompactSharesReturnsErrForShareWithStartIndicatorFalse(t *testing.T) {
	txs := generateRandomTransaction(2, appconsts.CompactContinuationShareContentSize*4)
	shares := SplitTxs(txs)
