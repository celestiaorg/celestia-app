...
func TestValidateBlobNamespace(t *testing.T) {
	type testCase struct {
		name    string
		ns      Namespace
		wantErr bool
	...
	validBlobNamespace := MustNewV0(bytes.Repeat([]byte{1}, NamespaceVersionZeroIDSize))
	testCases := []testCase{
		{
			name:    "valid blob namespace",
			ns:      validBlobNamespace,
			wantErr: false,
		},
		{
			name: "unsupported namespace version: 1",
			ns: Namespace{
				Version: uint8(1),
				ID:      validBlobNamespace.ID,
			},
			wantErr: true,
		},
		{
			name: "unsupported namespace version: max",
			ns: Namespace{
				Version: NamespaceVersionMax,
				ID:      validBlobNamespace.ID,
			},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			err := tc.ns.ValidateBlobNamespace()
			if tc.wantErr {
				assert.Error(t, err)
				return
			}
