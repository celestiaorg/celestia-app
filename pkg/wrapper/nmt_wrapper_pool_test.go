...
func TestFixedTreePool_AcquireRelease(t *testing.T) {
	squareSize := uint64(8)
	poolSize := 4
	pool := wrapper.NewTreePool(squareSize, poolSize)

	trees := make([]*wrapper.ErasuredNamespacedMerkleTree, 0, poolSize)

	for i := 0; i < poolSize; i++ {
		constructor := pool.NewConstructor()
		tree := constructor(rsmt2d.Row, uint(i))
		nmt, ok := tree.(*wrapper.ErasuredNamespacedMerkleTree)
		require.True(t, ok)
...
func TestBufferedTree_WithPoolReuse(t *testing.T) {
	...
	if nmt, ok := tree.(*wrapper.ErasuredNamespacedMerkleTree); ok {
		nmt.Release()
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			edsWithoutPool, err := rsmt2d.ComputeExtendedDataSquare(
				...
				wrapper.NewConstructor(uint64(tc.squareSize)),
			)
			require.NoError(t, err)

			pool := wrapper.NewTreePool(uint64(tc.squareSize), runtime.NumCPU()*4)
			edsWithPool, err := rsmt2d.ComputeExtendedDataSquareLimitParallelOps(
				data,
				appconsts.DefaultCodec(),
				pool.NewConstructor(),
				pool.PoolSize(),
			...
			dahWithoutPool, err := da.NewDataAvailabilityHeader(edsWithoutPool)
			require.NoError(t, err)
			dahWithPool, err := da.NewDataAvailabilityHeader(edsWithPool)
			require.NoError(t, err)

			assert.True(t, bytes.Equal(dahWithoutPool.Hash(), dahWithPool.Hash()),
				"DAH hash should be equal")
		})
	}
}

func TestComputeExtendedDataSquare_UsingExtendSharesWithTreeFactory(t *testing.T) {
	testCases := []struct {
		name       string
		squareSize int
	}{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			edsWithoutPool, err := da.ExtendShares(data)
			require.NoError(t, err)

			pool := wrapper.NewTreePool(uint64(tc.squareSize), runtime.NumCPU()*4)
			edsWithPool, err := da.ExtendSharesWithTreePool(data, pool)
			require.NoError(t, err)
...
func TestErasuredNamespacedMerkleTree_FastRoot(t *testing.T) {
	squareSize := uint64(8)
	tree := wrapper.NewErasuredNamespacedMerkleTree(squareSize, 0)
	...
	tree2 := wrapper.NewErasuredNamespacedMerkleTree(squareSize, 0)
	for _, d := range data {
		err := tree2.Push(d)
		require.NoError(t, err)
	}

	fastRoot, err := tree2.FastRoot()
	require.NoError(t, err)

	assert.True(t, bytes.Equal(root, fastRoot), "FastRoot should produce the same root as Root")
}
...
func BenchmarkExtendedDataSquare_WithPool(b *testing.B) {
	...
	for _, size := range squareSizes {
		b.Run(fmt.Sprintf("SquareSize-%d", size), func(b *testing.B) {
			...
			pool := wrapper.NewTreePool(uint64(size), runtime.NumCPU()*4)

			b.ReportAllocs()
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				square, err := rsmt2d.ComputeExtendedDataSquareLimitParallelOps(
					data,
					appconsts.DefaultCodec(),
					pool.NewConstructor(),
					pool.PoolSize(),
