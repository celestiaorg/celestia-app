...
func MinorVersionCompatibility(logger *log.Logger) error {
	...
	testnet, err := testnets.New("runMinorVersionCompatibility", seed, nil, testnets.GetTestDefaultSetting())
...
func MajorUpgradeToV2(logger *log.Logger) error {
	...
	testnet, err := testnets.New("runMajorUpgradeToV2", seed, nil, testnets.GetTestDefaultSetting())
	testnets.NoError("failed to create testnet", err)
