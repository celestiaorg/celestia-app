...
func MinorVersionCompatibility(logger *log.Logger) error {
	...
	testnet, err := testnets.New("runMinorVersionCompatibility", seed, nil, testnets.GetTestnetDefaultSetting())
...
func MajorUpgradeToV2(logger *log.Logger) error {
	...
	testnet, err := testnets.New("runMajorUpgradeToV2", seed, nil, testnets.GetTestnetDefaultSetting())
	testnets.NoError("failed to create testnet", err)
