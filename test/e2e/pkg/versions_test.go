...
func TestVersionParsing(t *testing.T) {
	...
	versions := pkg.ParseVersions(versionStr)
	require.Len(t, versions, 3)
	require.Len(t, versions.FilterOutReleaseCandidates(), 2)
	require.Equal(t, versions.GetLatest(), pkg.Version{1, 3, 0, false, 0})
}
...
func TestOrder(t *testing.T) {
	...
	versions := pkg.ParseVersions(versionStr)
	versions.Order()
	require.Equal(t, versions[0], pkg.Version{1, 1, 0, false, 0})
	require.Equal(t, versions[1], pkg.Version{1, 2, 0, true, 0})
	require.Equal(t, versions[2], pkg.Version{1, 2, 1, false, 0})
	require.Equal(t, versions[3], pkg.Version{1, 3, 0, false, 0})
	require.Equal(t, versions[4], pkg.Version{1, 4, 0, false, 0})
	require.Equal(t, versions[5], pkg.Version{2, 0, 0, false, 0})
...
func TestOrderOfReleaseCandidates(t *testing.T) {
	...
	versions := pkg.ParseVersions(versionsStr)
	versions.Order()
	require.Equal(t, versions[0], pkg.Version{1, 0, 0, true, 0})
	require.Equal(t, versions[1], pkg.Version{1, 0, 0, true, 1})
	require.Equal(t, versions[2], pkg.Version{1, 0, 0, false, 0})
}
