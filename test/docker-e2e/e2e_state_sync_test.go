			...
			for attempt := 0; attempt < maxRetries && !success; attempt++ {
				...
				if err != nil {
					...
					if strings.Contains(err.Error(), "connection") || strings.Contains(err.Error(), "rpc error") || strings.Contains(err.Error(), "Unavailable") {
						...
						// Non-connection error, log and move on
...
func (s *CelestiaTestSuite) TestStateSyncAcrossMixedAppVersions() {
	...
	// Configure genesis for v4 with 4 validators total (default has 2, add 2 more)
	cfg.Genesis = cfg.Genesis.
		WithAppVersion(4).
		WithValidators(
			genesis.NewDefaultValidator("validator3"),
			genesis.NewDefaultValidator("validator4"),
		)

	chain, err := dockerchain.NewCelestiaChainBuilder(s.T(), cfg).
		WithPostInit(validatorStateSyncAppOverrides). // Enable snapshots
		Build(ctx)
	...
	// Verify chain is running and get initial state
	height, err := chain.Height(ctx)
	s.Require().NoError(err, "failed to get chain height")
	s.Require().Greater(height, int64(0), "chain height should be greater than 0")
	...
	stableHeight, err := s.GetLatestBlockHeight(ctx, rpcClient)
	s.Require().NoError(err, "failed to get stable height")

	targetHeight := stableHeight + 20
	s.Require().NoError(wait.ForBlocks(ctx, int(targetHeight-stableHeight), chain),
		"failed to wait for 20 blocks")

	t.Logf("Successfully produced %d blocks at app version 4", targetHeight-stableHeight)

	// Test functionality before upgrade to ensure gRPC is working
	t.Log("Testing functionality before upgrade")

	// Wait a bit more for gRPC to be fully ready (TxSim is already connected, so gRPC is working)
	...
	// Produce 50 more blocks at v5
	t.Log("Producing 50 more blocks at app version 5")
	s.Require().NoError(wait.ForBlocks(ctx, 50, chain),
		"failed to wait for 50 blocks after upgrade")
	...
	s.Require().Equal("fn", stateSyncNode.GetType(), "expected state sync node to be a full node")
