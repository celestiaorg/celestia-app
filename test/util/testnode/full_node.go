...
func DefaultGenesisState(fundedAccounts ...string) (map[string]json.RawMessage, keyring.Keyring, error) {
	...
	var bsGenState bstypes.GenesisState
	encCfg.Codec.MustUnmarshalJSON(state[bstypes.ModuleName], &bsGenState)
	bsGenState.Params.DataCommitmentWindow = bstypes.MinimumDataCommitmentWindow
	state[bstypes.ModuleName] = encCfg.Codec.MustMarshalJSON(&bsGenState)
