...
func TracingConfigurator(runenv *runtime.RunEnv, tparams TracingParams) Configurator {
	return func(nodes []RoleConfig) ([]RoleConfig, error) {
		...
		for i := range nodes {
			...
			nodes[i].CmtConfig.Instrumentation.InfluxTables = strings.Join(schema.MempoolTables(), ",")
		}
		...
		for i := 0; i < tparams.Nodes; i++ {
			nodes[i].CmtConfig.Instrumentation.InfluxTables = strings.Join([]string{schema.BlockTable, schema.MempoolTxTable, schema.MempoolPeerStateTable, schema.MempoolRejectedTable}, ",")
