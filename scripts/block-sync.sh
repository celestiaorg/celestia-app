...
# Unified block sync script for Celestia networks (Arabica, Mocha)
# Usage: ./block-sync.sh --network <arabica|mocha>
...
usage() {
  echo "Usage: $0 --network <arabica|mocha>"
  exit 1
}
...
case "$NETWORK" in
  arabica)
    ...
    START_CMD="celestia-appd start --force-no-bbr"
    ;;
  mocha)
    CHAIN_ID="mocha-4"
    ...
    # Mocha requires --v2-upgrade-height
    START_CMD="celestia-appd start --v2-upgrade-height 2585031 --force-no-bbr"
    ;;
  *)
