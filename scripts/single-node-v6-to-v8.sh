...
# This script starts a local single node testnet on app version 6 and then
# upgrades directly to app version 8 by signalling v8 from the single
# validator (100% voting power) and submitting MsgTryUpgrade.
#
# Prerequisites:
# - Check out the v8.x branch.
# - Run `make install` so celestia-appd embeds v3-v7 and runs v8 natively.
...
upgrade() {
    ...
    echo "Waiting for the chain to activate app version ${TO_VERSION}..."
    while true; do
        current_version=$(celestia-appd status 2>/dev/null | jq -r '.node_info.protocol_version.app')
        if [ "$current_version" = "${TO_VERSION}" ]; then
            echo "Upgrade to version ${TO_VERSION} complete!"
            break
        fi
        echo "Current version: ${current_version:-unknown}, waiting for version ${TO_VERSION}..."
        sleep 2
    done
}
...
upgrade & # Start the upgrade process to v8 in the background.
