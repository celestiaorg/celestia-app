...
lazy_static! {
    /// Total number of transactions submitted (broadcast started).
    ...
    pub static ref TX_FAILED: Counter = register_counter!(
        "latency_monitor_tx_failed_total",
...
/// Record a successful transaction confirmation with its latency.
pub fn record_confirm(latency: Duration) {
    TX_CONFIRMED.inc();
    TX_IN_FLIGHT.dec();
    LATENCY_HISTOGRAM.observe(latency.as_secs_f64());
}

/// Record a failed transaction confirmation.
pub fn record_failure() {
    TX_FAILED.inc();
    TX_IN_FLIGHT.dec();
}

/// Handle HTTP requests - serve metrics on /metrics, 404 otherwise.
async fn handle_request(
    ...
    match req.uri().path() {
        ...
        "/" => {
            let body = "<html><body><h1>Lumina Latency Monitor</h1><p><a href=\"/metrics\">Metrics</a></p></body></html>";
            Ok(Response::builder()
                .status(StatusCode::OK)
