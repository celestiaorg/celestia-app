...
func Run(ctx context.Context, cfg BuilderConfig, dir string) error {
	...
	blockDB, err := dbm.NewDB("blockstore", dbm.BadgerDBBackend, tmCfg.DBDir())
	if err != nil {
	...
	stateDB, err := dbm.NewDB("state", dbm.BadgerDBBackend, tmCfg.DBDir())
	if err != nil {
	...
	appDB, err := tmdbm.NewDB("application", tmdbm.BadgerDBBackend, tmCfg.DBDir())
	if err != nil {
