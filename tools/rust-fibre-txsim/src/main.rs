    ...
    loop {
        tokio::select! {
            Some(result) = upload_tasks.join_next(), if !upload_tasks.is_empty() => {
                let (account, result) = match result {
                    Ok(v) => v,
                    Err(e) => {
                        stats.failures.fetch_add(1, Ordering::Relaxed);
                        eprintln!("upload task join error: {e}");
                        continue;
                    }
                };
            ...
            Some(result) = submit_tasks.join_next(), if !submit_tasks.is_empty() => {
                let submit_result = match result {
                    ...
                    Err(e) => {
                        stats.failures.fetch_add(1, Ordering::Relaxed);
                        eprintln!("submit task join error: {e}");
                        continue;
                    }
                };
                match submit_result {
            ...
            Some(result) = confirm_tasks.join_next(), if !confirm_tasks.is_empty() => {
                let out = match result {
                    ...
                    Err(e) => {
                        stats.failures.fetch_add(1, Ordering::Relaxed);
                        eprintln!("confirm task join error: {e}");
                        continue;
                    }
                };
                match out.result {
...
async fn do_submit(
    ...
    // Upstream `StopError` doesn't implement `std::error::Error`, so we can't
    // use `.context()`. Use Debug formatting so inner variants aren't lost.
    let confirm: Pin<Box<dyn Future<Output = anyhow::Result<TxInfo>> + Send>> = Box::pin(async move {
        handle
            .confirm()
            .await
