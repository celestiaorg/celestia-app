...
type PublicInputs struct {
	...
	// NewHeaderHash is the block header hash of the latest state update.
	NewHeaderHash [32]byte
	// PreviousHeaderHash is the block header hash of the previous state.
	PreviousHeaderHash [32]byte
	// CelestiaHeaderHashes is the ordered set of Celestia header hashes whose
	// data roots contain the blobs posted between the previous and new headers.
	// TODO: A single hash (latest celestia header hash) should be possible to use here with further changes to proof system.
...
func (pi *PublicInputs) Marshal() ([]byte, error) {
	...
	if err := writeBytes(&buf, pi.NewHeaderHash[:]); err != nil {
		return nil, fmt.Errorf("write NewHeaderHash: %w", err)
	}
	if err := writeBytes(&buf, pi.PreviousHeaderHash[:]); err != nil {
		return nil, fmt.Errorf("write PreviousHeaderHash: %w", err)
	}

	if err := binary.Write(&buf, binary.LittleEndian, uint64(len(pi.CelestiaHeaderHashes))); err != nil {
		return nil, fmt.Errorf("write CelestiaHeaderHashes length: %w", err)
	}

	for i, hash := range pi.CelestiaHeaderHashes {
		if len(hash) != 32 {
			return nil, fmt.Errorf("CelestiaHeaderHashes[%d] has invalid length %d (expected 32)", i, len(hash))
		}
		if err := writeBytes(&buf, hash); err != nil {
			return nil, fmt.Errorf("write CelestiaHeaderHashes[%d]: %w", i, err)
		}
	}
...
func (pi *PublicInputs) Unmarshal(data []byte) error {
	...
	if err := readBytes(buf, pi.NewHeaderHash[:]); err != nil {
		return fmt.Errorf("read NewHeaderHash: %w", err)
	}
	if err := readBytes(buf, pi.PreviousHeaderHash[:]); err != nil {
		return fmt.Errorf("read PreviousHeaderHash: %w", err)
	}

	var numHeaderHashes uint64
	if err := binary.Read(buf, binary.LittleEndian, &numHeaderHashes); err != nil {
		return fmt.Errorf("read CelestiaHeaderHashes length: %w", err)
	}

	pi.CelestiaHeaderHashes = make([][]byte, numHeaderHashes)
	for i := uint64(0); i < numHeaderHashes; i++ {
		hash := make([]byte, 32)
		if _, err := io.ReadFull(buf, hash); err != nil {
			return fmt.Errorf("read CelestiaHeaderHashes[%d]: %w", i, err)
		}

		pi.CelestiaHeaderHashes[i] = hash
	}
