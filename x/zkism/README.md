...
- `isms` (collections.Map[uint64, EvolveEvmISM], `types.IsmsKeyPrefix`): Stores per‑ISM records containing trusted state and verifier configuration used during proof verification.
...
- CreateEvolveEvmISM: Creates an ISM with initial trusted state and verifier configuration.
- UpdateEvolveEvmISM: Verifies a state transition proof, then updates the ISM’s trusted `state_root` and `height`.
...
1) Off‑chain prover generates a state transition proof; submit via `UpdateEvolveEvmISM`.
...
- `Ism(id) -> EvolveEvmISM` — `GET /celestia/zkism/v1/isms/{id}`
- `Isms(pagination) -> [EvolveEvmISM]` — `GET /celestia/zkism/v1/isms`
- `Params() -> Params` — `GET /celestia/zkism/v1/params`

## Events

- `EventCreateEvolveEvmISM` emitted on creation with all ISM fields.
