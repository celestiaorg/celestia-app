...
An address can additionally commit to a specific post-dispatch hook. This makes the hook part of
the depositor's intent rather than a parameter chosen by whoever submits `MsgForward`:

```text
callDigest   = sha256(destDomain_32bytes || destRecipient || tokenId || hookId)
salt         = sha256(version_byte || callDigest)       // version_byte = 0x02
forwardAddr  = address.Module("forwarding", salt)[:20]
```

The two schemes are mutually exclusive and cannot collide, since the version byte and the digest
preimage change together:

- An address derived **without** a hook (`0x01`) can only be forwarded through the mailbox default
  hook. Supplying any `custom_hook_id` derives a different address and fails with `ErrAddressMismatch`.
- An address derived **with** a hook (`0x02`) can only be forwarded through that exact hook.
  Supplying a different hook, or omitting it, likewise fails with `ErrAddressMismatch`.

A zero `hookId` is not a valid binding — it is the sentinel for "mailbox default hook", which the
`0x01` scheme already covers. `DeriveForwardingAddressWithHook` rejects it with `ErrInvalidHookID`.
...
**The hook must match the one the forwarding address was derived with.** Choosing a hook is done at
address-derivation time, not at forward time — see [Hook-bound addresses](#hook-bound-addresses).
To route through your own IGP, derive the address with `--custom-hook-id`, have the user deposit
there, and pass the same hook id to `tx forwarding forward`.
...
- **Cryptographic binding**: The `forwardAddr` cryptographically commits to `(destDomain, destRecipient, tokenId)`, and to `hookId` when one is used. Funds can only be forwarded using the committed token route, destination, and post-dispatch hook.
- **Permissionless execution**: Anyone can trigger forwarding, but only to the pre-committed destination and only through the pre-committed hook.
