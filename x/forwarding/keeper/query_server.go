...
func (q queryServer) QuoteForwardingFee(ctx context.Context, req *types.QueryQuoteForwardingFeeRequest) (*types.QueryQuoteForwardingFeeResponse, error) {
	...
	// Reject overrides because forwarding always uses the mailbox default hook.
	if err := types.ValidateCustomHook(req.CustomHookId, req.CustomHookMetadata); err != nil {
