...
func (suite *MsgServerTestSuite) TestPayForFibreWithPendingWithdrawals() {
	...
	suite.T().Run("multiple withdrawals oldest consumed first", func(t *testing.T) {
		newSignerPubKey, newPrivKey, newSigner := suite.newSigner()
		paymentPromise := suite.createPaymentPromise(newSignerPubKey, newPrivKey)
		gasRequired := uint64(paymentPromise.BlobSize) * uint64(params.GasPerBlobByte)
		// gasRequired = 1000. Balance covers payment, all locked in withdrawals.
		totalBalance := int64(gasRequired) + 500 // 1500

		// Create escrow account: all balance locked in withdrawals
		escrowAccount := types.EscrowAccount{
			Signer:           newSigner,
			Balance:          sdk.NewInt64Coin(appconsts.BondDenom, totalBalance),
			AvailableBalance: sdk.NewInt64Coin(appconsts.BondDenom, 0),
		}
		suite.keeper.SetEscrowAccount(suite.ctx, escrowAccount)

		// Create 3 withdrawals at different timestamps totaling 1500
		// w1=200, w2=300, w3=1000
		t1 := suite.ctx.BlockTime()
		t2 := suite.ctx.BlockTime().Add(1 * time.Second)
		t3 := suite.ctx.BlockTime().Add(2 * time.Second)

		w1 := types.Withdrawal{
			Signer:             newSigner,
			Amount:             sdk.NewInt64Coin(appconsts.BondDenom, 200),
			RequestedTimestamp: t1,
			AvailableTimestamp: t1.Add(params.WithdrawalDelay),
		}
		w2 := types.Withdrawal{
			Signer:             newSigner,
			Amount:             sdk.NewInt64Coin(appconsts.BondDenom, 300),
			RequestedTimestamp: t2,
			AvailableTimestamp: t2.Add(params.WithdrawalDelay),
		}
		w3 := types.Withdrawal{
			Signer:             newSigner,
			Amount:             sdk.NewInt64Coin(appconsts.BondDenom, 1000),
			RequestedTimestamp: t3,
			AvailableTimestamp: t3.Add(params.WithdrawalDelay),
		}
		suite.keeper.SetWithdrawal(suite.ctx, w1)
		suite.keeper.SetWithdrawal(suite.ctx, w2)
		suite.keeper.SetWithdrawal(suite.ctx, w3)
		...
		// paymentAmount = 1000, shortfall = 1000 (AvailableBalance was 0).
		// Oldest first: w1(200) fully consumed → remaining 800,
		//               w2(300) fully consumed → remaining 500,
		//               w3(1000) partially reduced to 500.

		// w1 should be fully consumed (deleted)
		_, found := suite.keeper.GetWithdrawal(suite.ctx, newSigner, t1)
		require.False(t, found, "w1 should be fully consumed")

		// w2 should be fully consumed (deleted)
		_, found = suite.keeper.GetWithdrawal(suite.ctx, newSigner, t2)
		require.False(t, found, "w2 should be fully consumed")

		// w3 should be partially reduced: 1000-500 = 500
		updatedW3, found := suite.keeper.GetWithdrawal(suite.ctx, newSigner, t3)
		require.True(t, found, "w3 should still exist (partially reduced)")
		require.Equal(t, sdk.NewInt64Coin(appconsts.BondDenom, 500), updatedW3.Amount)

		// Verify escrow balance
		updatedAccount, found := suite.keeper.GetEscrowAccount(suite.ctx, newSigner)
		require.True(t, found)
		require.Equal(t, sdk.NewInt64Coin(appconsts.BondDenom, 500), updatedAccount.Balance)
		require.Equal(t, sdk.NewInt64Coin(appconsts.BondDenom, 0), updatedAccount.AvailableBalance)
	})

	suite.T().Run("partial available balance covers some of payment", func(t *testing.T) {
		partialSignerPubKey, partialPrivKey, partialSigner := suite.newSigner()
		paymentPromise := suite.createPaymentPromise(partialSignerPubKey, partialPrivKey)
		gasRequired := uint64(paymentPromise.BlobSize) * uint64(params.GasPerBlobByte)
		paymentAmount := int64(gasRequired)
		partialAvailable := paymentAmount / 2

		// Create escrow account with partial available balance
		escrowAccount := types.EscrowAccount{
			Signer:           partialSigner,
			Balance:          sdk.NewInt64Coin(appconsts.BondDenom, paymentAmount+1000),
			AvailableBalance: sdk.NewInt64Coin(appconsts.BondDenom, partialAvailable),
		}
		suite.keeper.SetEscrowAccount(suite.ctx, escrowAccount)

		// Create a withdrawal for the locked portion
		withdrawalAmount := paymentAmount + 1000 - partialAvailable
		withdrawal := types.Withdrawal{
			Signer:             partialSigner,
			Amount:             sdk.NewInt64Coin(appconsts.BondDenom, withdrawalAmount),
			RequestedTimestamp: suite.ctx.BlockTime(),
			AvailableTimestamp: suite.ctx.BlockTime().Add(params.WithdrawalDelay),
		}
		suite.keeper.SetWithdrawal(suite.ctx, withdrawal)
		...
		// Verify balances
		updatedAccount, found := suite.keeper.GetEscrowAccount(suite.ctx, partialSigner)
		suite.True(found)
		suite.Equal(sdk.NewInt64Coin(appconsts.BondDenom, 1000), updatedAccount.Balance)
		suite.Equal(sdk.NewInt64Coin(appconsts.BondDenom, 0), updatedAccount.AvailableBalance)

		// Verify withdrawal was reduced by the shortfall (paymentAmount - partialAvailable)
		shortfall := paymentAmount - partialAvailable
		expectedWithdrawalAmount := withdrawalAmount - shortfall
		updatedWithdrawal, found := suite.keeper.GetWithdrawal(suite.ctx, partialSigner, suite.ctx.BlockTime())
		suite.True(found)
		suite.Equal(sdk.NewInt64Coin(appconsts.BondDenom, expectedWithdrawalAmount), updatedWithdrawal.Amount)
	})
}
