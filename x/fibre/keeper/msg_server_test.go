...
// TestRetentionWindowShorterThanWithdrawalDelayAllowsReplay pins down why
// Params.Validate rejects a payment_promise_retention_window shorter than the
// withdrawal_delay. Under such a configuration the replay record is pruned
// while the promise is still fresh enough to settle again, so the escrow owner
// is charged twice for a single blob.
func (suite *MsgServerTestSuite) TestRetentionWindowShorterThanWithdrawalDelayAllowsReplay() {
	const (
		withdrawalDelay = 24 * time.Hour
		promiseTimeout  = time.Hour
		// Shorter than withdrawalDelay, which is what opens the replay window.
		retentionWindow = time.Hour
	)

	// SetParams does not run Params.Validate, standing in for a chain that
	// installed this configuration before the invariant was enforced.
	suite.keeper.SetParams(suite.ctx, types.NewParams(
		types.DefaultGasPerBlobByte,
		withdrawalDelay,
		promiseTimeout,
		retentionWindow,
		types.DefaultPaymentPromiseHeightWindow,
		types.DefaultShardRetention,
	))
	...
	// Fund the escrow for two settlements so the replay is not stopped by balance.
	gas := keeper.EstimateGasForPayForFibre(promise.BlobSize)
	payment := sdk.NewInt64Coin(appconsts.BondDenom, int64(gas))
	...
	// Move past the retention window so BeginBlocker prunes the replay record.
	// The promise is still well within withdrawal_delay of its creation time.
	suite.ctx = suite.ctx.WithBlockTime(promise.CreationTimestamp.Add(retentionWindow).Add(time.Minute))
	...
	suite.Require().False(found, "replay record should have been pruned")

	// The timeout path skips the expiry and height-window checks, so the
	// withdrawal_delay freshness check and the now-pruned replay record were the
	// only things standing between this promise and a second settlement.
	_, err = suite.msgServer.PaymentPromiseTimeout(suite.ctx, &types.MsgPaymentPromiseTimeout{
		Signer:         sdk.AccAddress(secp256k1.GenPrivKey().PubKey().Address()).String(),
		PaymentPromise: promise,
	})
	suite.NoError(err, "the same promise settled a second time")

	account, found := suite.keeper.GetEscrowAccount(suite.ctx, signer)
	suite.True(found)
	suite.Equal(balance.Sub(payment).Sub(payment).String(), account.Balance.String(), "escrow was charged twice for one promise")
}
