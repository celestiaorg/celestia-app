...
// TestFeeAddressSendAndForward verifies the fee address balance lifecycle:
// 1. Fee address starts with zero balance
// 2. After MsgSend and MsgForwardFees, fee address balance returns to zero
// Note: The intermediate state (fee address has sendAmount after MsgSend but before
// MsgForwardFees) cannot be reliably verified because testnode auto-advances blocks.
...
func (s *IntegrationTestSuite) TestFeeAddressRejectsNonUtia() {
	...
	require.ErrorContains(err, "only utia can be sent to fee address")
}
...
func (s *IntegrationTestSuite) TestZeroBalanceTransition() {
	...
	// Initial state: fee address may have balance from previous tests
	// Record initial balance to verify transitions work regardless of starting state
	initialBalance := s.getFeeAddressBalance()
	require.True(initialBalance.GTE(math.ZeroInt()), "initial balance should be >= 0")
