...
func CreateCommitment(namespace []byte, blobData []byte, shareVersion uint8) ([]byte, error) {
	...
	for i, set := range leafSets {
		...
		for _, leaf := range set {
			...
			// the nmt wrapper (pkg/wrapper). Each namespace is added for each
			// share as to keep the namespace in the share, and therefore the
			// parity data, while also allowing for the manual addition of the
			// parity namespace.
