...
![vacuum! map](./figures/recovery/vacuum!_map.png)
...
commits to. The size of the transaction can be calculated as `end - start`.

The `bp_hash` is generated by taking the merkle root over of each of the blob
transactions in `TxMetaData` and `Have` messages.
