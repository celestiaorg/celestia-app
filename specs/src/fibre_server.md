...
`StoreConfig.Path` is not a TOML field; the standalone `fibre start` command sets it from `--home`. The default state client is a gRPC app client connected to `AppGRPCAddress`. The default signer is a PrivValidatorAPI gRPC client connected to `SignerGRPCAddress`. App-node gRPC uses insecure local transport and is expected to be loopback or otherwise protected. Signer gRPC defaults to the same insecure local transport for a loopback endpoint. To reach a remote node, set `signer_grpc_ca_file`, `signer_grpc_cert_file` and `signer_grpc_key_file` together to enable mutual TLS: the CA verifies the node's server certificate and the client certificate is presented to the node.
