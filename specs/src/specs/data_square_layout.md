...
The `SubtreeRootThreshold` is an arbitrary versioned protocol constant that aims to put a soft limit on the number of subtree roots included in a blob inclusion proof, as described in [ADR-013](../../../docs/architecture/adr-013-non-interactive-default-rules-for-reduced-padding-2.md). A higher `SubtreeRootThreshold` means less padding and more tightly packed squares but also means greater proof sizes.
