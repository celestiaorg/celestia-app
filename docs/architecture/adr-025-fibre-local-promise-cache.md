...
- **SignerBudget**: `signer`, `anchor_available_balance`, `remaining_budget`, `last_reconcile_at`, `ops_since_reconcile`
- **PendingPromise**: `promise_hash`, `signer`, `amount`, `created_at`, `expires_at`

A signer-to-promises index allows reconciliation to list unresolved promises for a single signer without scanning all entries. Updates to SignerBudget and PendingPromise within a single reservation are written as an atomic DB batch.

#### Cache Eviction

If a signer has no new operations for longer than `PaymentPromiseTimeout + 1h` (i.e., all pending promises have either settled or expired and the timeout agent has had time to submit them), delete the signer's entire cache entry (SignerBudget and all PendingPromise records). This keeps the cache bounded to active signers only. The check is performed during reconciliation: if after dropping resolved/expired promises no pending promises remain and `now - last_reconcile_at` exceeds the eviction threshold, the signer is evicted. On the next validation for that signer, the cache is rebuilt from chain state.

#### Restart Behavior

On startup, the cache is loaded from DB as-is. No chain history rescan is performed. If a signer's cache entry is older than the reconciliation interval, it is marked stale and reconciled on that signer's next validation. This preserves unresolved reservations across restarts.
...
5. If budget is insufficient, perform a sweep-and-retry. If it still does not fit, reject with insufficient balance.
...
- Does not prevent the selective-validator attack. Clients can still skew per-validator caches by distributing promises across validators.
