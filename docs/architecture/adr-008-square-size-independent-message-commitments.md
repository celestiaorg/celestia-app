...
To implement this decision, you need to change [`CreateCommitment`](https://github.com/celestiaorg/celestia-app/v2/blob/8b9c4c9d13fe0ccb6ea936cc26dee3f52b6f6129/x/blob/types/payforblob.go#L111-L117).
In detail, [`powerOf2MountainRange`](https://github.com/celestiaorg/celestia-app/v2/blob/0c81704939cd743937aac2859f3cb5ae6368f174/x/payment/types/payfordata.go#L142) should take `msgMinSquareSize` as an argument instead of `squareSize`.
...
- Simplifies the creation of PFBs because users don't need to create commitments for multiple square sizes.
  ...
  - Simplifies the malleation process because this ADR enables a future refactor to remove WireMsgPayForBlob entirely ([issue](https://github.com/celestiaorg/celestia-app/v2/issues/951)). Previously multiple signatures were included in a WireMsgPayForBlob and only one was used to construct the MsgPayForBlob that ended up on-chain but this ADR results in only one signature needed on the wrapping SDK message and the SDK message that ends up on-chain.
  - This renders the following issues obsolete:
    - <https://github.com/celestiaorg/celestia-app/v2/issues/236>
    - <https://github.com/celestiaorg/celestia-app/v2/issues/727>
