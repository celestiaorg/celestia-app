...
func (app *App) ValidateBlobTxWithCache(blobTx *blobtx.BlobTx) (bool, error) {
	exists, expectedBlobHash := app.txCache.Exists(blobTx.Tx)
	if exists {
		blobHash := app.txCache.getBlobsHash(blobTx.Blobs)
		if expectedBlobHash != blobHash {
			return false, fmt.Errorf("proposed blob hash does not match cached blob hash: %s != %s", blobHash, expectedBlobHash)
		}
