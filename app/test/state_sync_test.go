...
func TestStateSync(t *testing.T) {
	t.Run("OfferSnapshot v1", func(t *testing.T) {
		testApp := createTestApp(t)
		resp := testApp.OfferSnapshot(abcitypes.RequestOfferSnapshot{ // fails because app.snapshotManager is not defined
	...
	t.Run("OfferSnapshot v2", func(t *testing.T) {
		testApp := createTestApp(t)
		resp := testApp.OfferSnapshot(abcitypes.RequestOfferSnapshot{ // fails because app.snapshotManager is not defined
			Snapshot: &abcitypes.Snapshot{
				Height:     0x290400,
...
func createTestApp(t *testing.T) *app.App {
	...
	require.NoError(t, err)
	testApp := app.New(log.NewNopLogger(), db, nil, 0, encCfg, upgradeHeight, util.EmptyAppOptions{}, baseapp.SetSnapshot(snapshotStore, snapshottypes.NewSnapshotOptions(10, 10)))
	require.NoError(t, err)
	infoResp := testApp.Info(abcitypes.RequestInfo{})
	require.Equal(t, uint64(0), infoResp.AppVersion)
	return testApp
}
