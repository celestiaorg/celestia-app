...
//
// ExportAppStateAndValidators built its context with app.NewContext, which uses
// an empty header and therefore reports a block height of 0.
// CalculateDelegationRewards only applies a validator's slash events when
//
//	endingHeight (= ctx.BlockHeight()) > startingHeight (= starting_info.height)
//
// so at height 0 no slash was ever applied. The accumulated stake stayed at its
// pre-slash value, ended up larger than TokensFromShares by the slash fraction,
// exceeded the 3*SmallestDec tolerance, and panicked.
//
// The delegation has to be created above height 0: a delegation whose
// starting_info.height is 0 hits the "started this height, no rewards yet"
// early return and never reaches the sanity check. That is why
// TestExportAppStateAndValidators above never caught this.
func TestExportForZeroHeightAfterSlashing(t *testing.T) {
	...
	// stake now exceeds what its shares are actually worth. This is the same
	// state 1,275 of mocha-4's 3,746 delegations are in.
	execBlock(t, testApp, func(ctx sdk.Context) {
