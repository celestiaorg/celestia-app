...
func TestFilteredSquareBuilderFillMaxTxBytesFibre(t *testing.T) {
	encConf := encoding.MakeConfig(ModuleEncodingRegisters...)
	txConfig := encConf.TxConfig
	...
	// Three pay-for-fibre txs of identical size for cumulative max tx bytes tests.
	pff1 := blobfactory.UnsignedPayForFibreTx(t, txConfig)
	pff2 := blobfactory.UnsignedPayForFibreTx(t, txConfig)
	pff3 := blobfactory.UnsignedPayForFibreTx(t, txConfig)
	require.Equal(t, len(pff1), len(pff2))
	require.Equal(t, len(pff1), len(pff3))
	pffSize := int64(len(pff1))

	// A normal tx used to verify that max tx bytes is shared across normal and
	// fibre txs: bytes already used by an accepted normal tx must be counted
	// when deciding whether a later fibre tx fits.
	normalTx := newNormalTx(t, txConfig)
	...
	tests := []struct {
		...
		{
			name:       "all fibre txs fit when max tx bytes is unbounded",
			txs:        [][]byte{pff1, pff2, pff3},
			maxTxBytes: math.MaxInt64,
			wantKept:   3,
		},
		{
			name:       "first fibre tx alone exceeds max tx bytes",
			txs:        [][]byte{pff1, pff2, pff3},
			maxTxBytes: pffSize - 1,
			wantKept:   0,
		},
		{
			name:       "max tx bytes fits exactly two fibre txs",
			txs:        [][]byte{pff1, pff2, pff3},
			maxTxBytes: 2 * pffSize,
			wantKept:   2,
		},
		{
			name:       "fibre tx skipped because normal tx consumed the remaining bytes",
			txs:        [][]byte{normalTx, pff1},
			maxTxBytes: int64(len(normalTx)),
			wantKept:   1,
		},
	}
