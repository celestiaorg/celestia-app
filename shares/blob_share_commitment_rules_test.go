...
func TestFitsInSquare(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			res, _ := FitsInSquare(tt.start, tt.size, 64, tt.blobs...)
...
func TestNextShareIndex(t *testing.T) {
	...
	tests := []test{
		...
		},
		{
			...
			cursor:        11,
			blobLen:       64,
			squareSize:    RoundUpPowerOfTwo(64),
			expectedIndex: 11,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			res := NextShareIndex(tt.cursor, tt.blobLen, 64)
...
func TestSubTreeWidth(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("shareCount %d", tc.shareCount), func(t *testing.T) {
			got := SubTreeWidth(tc.shareCount, 64)
