...
func TestCount(t *testing.T) {
	...
	testCases := []testCase{
		...
		{transactions: []coretypes.Tx{bytes.Repeat([]byte{1}, RawTxSize(FirstCompactShareContentSize+1))}, wantShareCount: 2},
	...
	continuationShare, _ := zeroPadIfNecessary(
		append(
			...
			append(
				...
				}, bytes.Repeat([]byte{0xf}, NamespaceSize+ShareInfoBytes+SequenceLenBytes+CompactShareReservedBytes)..., // data
			)...,
		),
		ShareSize)
...
func TestExport(t *testing.T) {
	...
	exactlyOneShare := coretypes.Tx(bytes.Repeat([]byte{4}, RawTxSize(FirstCompactShareContentSize)))
	exactlyTwoShares := coretypes.Tx(bytes.Repeat([]byte{5}, RawTxSize(FirstCompactShareContentSize+ContinuationCompactShareContentSize)))
