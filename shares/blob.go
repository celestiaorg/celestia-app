...
func NewBlob(ns Namespace, blob []byte, shareVersion uint8) *Blob {
	return &Blob{
		NamespaceId:      ns.ID,
		Data:             blob,
...
func (b *Blob) Validate() error {
	...
	if len(b.NamespaceId) != NamespaceIDSize {
		return fmt.Errorf("namespace id must be %d bytes", NamespaceIDSize)
	}
	if b.ShareVersion > math.MaxUint8 {
		return errors.New("share version can not be greater than MaxShareVersion")
	}
	if b.NamespaceVersion > NamespaceVersionMax {
